...
def internal_error(error):
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>500 - সার্ভার ত্রুটি</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
        <style>
            body {
                background: linear-gradient(135deg, #0f0c29, #302b63, #24243e);
...
# ==================== সিকিউরিটি হেডার মিডলওয়্যার (নতুন) ====================
@app.after_request
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>হোস্টিং প্যানেল - {% block title %}{% endblock %}</title>
...
<body>
    ...
    <div class="d-flex flex-wrap">
        ...
        <nav class="sidebar p-3">
            ...
            <div class="nav flex-column">
                <a href="/dashboard" class="nav-link {{ 'active' if request.path == '/dashboard' }}">
                    <i class="fas fa-tachometer-alt"></i> ড্যাশবোর্ড
                </a>
                <a href="/file-manager" class="nav-link {{ 'active' if request.path == '/file-manager' }}">
                    <i class="fas fa-folder"></i> ফাইল ম্যানেজার
                </a>
                <a href="/project-manager" class="nav-link {{ 'active' if request.path == '/project-manager' }}">
                    <i class="fas fa-project-diagram"></i> প্রজেক্ট ম্যানেজার
                </a>
                <a href="/searxng" class="nav-link {{ 'active' if request.path == '/searxng' }}">
                    <i class="fas fa-search"></i> SearXNG
                </a>
                <a href="/settings" class="nav-link {{ 'active' if request.path == '/settings' }}">
                    <i class="fas fa-cog"></i> সেটিংস
                </a>
                <a href="/profile" class="nav-link {{ 'active' if request.path == '/profile' }}">
                    <i class="fas fa-user"></i> প্রোফাইল
                </a>
                <a href="/logs" class="nav-link {{ 'active' if request.path == '/logs' }}">
                    <i class="fas fa-history"></i> লগ
                </a>
                <a href="/server-info" class="nav-link {{ 'active' if request.path == '/server-info' }}">
    ...
    <!-- কমন JavaScript ফাংশন (সংশোধিত) -->
    <script>
        // ==================== CSRF টোকেন ম্যানেজমেন্ট (নতুন) ====================
        ...
        function hideLoading() {
            // মিনিমাম 500ms দেখাবে যাতে ফ্লিকার না হয়
            loadingTimeout = setTimeout(() => {
                document.getElementById('loadingOverlay').style.display = 'none';
            }, 500);
        }
        
        // ==================== AJAX হেল্পার (সংশোধিত) ====================
...
def dashboard():
        ...
        <div class="row mb-4">
            <div class="col-md-3 mb-3">
                <div class="glass-card p-4 text-center">
                    ...
                    <h5>CPU ব্যবহার</h5>
                    <h3 class="fw-bold">{{ system_info.cpu_usage }}%</h3>
                    <small>{{ system_info.cpu_count }} কোর | {{ system_info.cpu_freq_current }}</small>
                    <div class="progress-glass mt-2">
                        <div class="progress-bar-glass" style="width: {{ system_info.cpu_usage }}%"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-3 mb-3">
                <div class="glass-card p-4 text-center">
                    ...
                    <h5>RAM ব্যবহার</h5>
                    <h3 class="fw-bold">{{ system_info.ram_percent }}%</h3>
                    <small>{{ system_info.used_ram }} / {{ system_info.total_ram }}</small>
                    <div class="progress-glass mt-2">
                        <div class="progress-bar-glass" style="width: {{ system_info.ram_percent }}%"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-3 mb-3">
                <div class="glass-card p-4 text-center">
                    ...
                    <h5>স্টোরেজ</h5>
                    <h3 class="fw-bold">{{ system_info.storage_percent }}%</h3>
                    <small>{{ system_info.used_storage }} / {{ system_info.total_storage }}</small>
                    <div class="progress-glass mt-2">
                        <div class="progress-bar-glass" style="width: {{ system_info.storage_percent }}%"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-3 mb-3">
                <div class="glass-card p-4 text-center">
                    ...
                    <h3 class="fw-bold">{{ system_info.uptime_formatted }}</h3>
                    <small>{{ system_info.hostname }}</small>
                </div>
            </div>
        </div>
        ...
        <div class="row mb-4">
            <div class="col-md-4 mb-3">
                <div class="glass-card p-4">
                    ...
                    <p class="mb-1">প্রেরিত: <strong>{{ system_info.network_sent }}</strong></p>
                    <p class="mb-0">গৃহীত: <strong>{{ system_info.network_recv }}</strong></p>
                </div>
            </div>
            <div class="col-md-4 mb-3">
                <div class="glass-card p-4">
                    <h6><i class="fas fa-exchange-alt"></i> ডিস্ক I/O</h6>
                    <p class="mb-1">পড়া: <strong>{{ system_info.disk_read_bytes }}</strong></p>
                    <p class="mb-0">লেখা: <strong>{{ system_info.disk_write_bytes }}</strong></p>
                </div>
            </div>
            <div class="col-md-4 mb-3">
                <div class="glass-card p-4">
                    <h6><i class="fas fa-memory"></i> সোয়াপ</h6>
                    <p class="mb-1">ব্যবহৃত: <strong>{{ system_info.swap_used }}</strong></p>
                    <p class="mb-0">মোট: <strong>{{ system_info.swap_total }}</strong></p>
                </div>
            </div>
        </div>
        ...
        <div class="glass-card p-4 mb-4">
            ...
            <div class="table-responsive">
                <table class="table table-dark table-hover">
                    ...
                    <tbody>
                        {% for app in system_info.running_apps %}
                        <tr>
                            <td><span class="badge bg-secondary">{{ app.pid }}</span></td>
                            <td>{{ app.name }}</td>
                            <td>
                                <span class="badge bg-primary">{{ app.cpu }}</span>
                            </td>
                            <td>
                                <span class="badge bg-success">{{ app.memory }}</span>
                            </td>
                            <td>
                            ...
                            <td>{{ app.username }}</td>
                            <td>
                                <button class="btn btn-sm btn-glass" onclick="manageApp('{{ app.pid }}')" title="ম্যানেজ করুন">
    ...
    <script>
        ...
        function manageApp(pid) {
            ...
            // পরবর্তীতে প্রসেস ম্যানেজমেন্ট যোগ করা হবে
        }
    </script>
    {% endblock %}
    '''
    
    return render_template_string(dashboard_html, system_info=system_info)
...
def file_manager():
    ...
    file_manager_html = '''
    {% extends "base" %}
    {% block title %}ফাইল ম্যানেজার{% endblock %}
    {% block content %}
    <div class="container-fluid">
        <h2 class="mb-4 fw-bold">
            <i class="fas fa-folder"></i> ফাইল ম্যানেজার
    ...
    return render_template_string(file_manager_html, items=items)
...
def searxng_page():
    ...
    searxng_html = '''
    {% extends "base" %}
    {% block title %}SearXNG ম্যানেজার{% endblock %}
    {% block content %}
    <div class="container-fluid">
        <h2 class="mb-4 fw-bold">
...
লোড হচ্ছে...
    ...
    return render_template_string(searxng_html)
...
def server_info():
    ...
    server_info_html = '''
    {% extends "base" %}
    {% block title %}সার্ভার তথ্য{% endblock %}
    {% block content %}
    <div class="container-fluid">
        <h2 class="mb-4 fw-bold">
            <i class="fas fa-info-circle"></i> সার্ভার তথ্য
            ...
            <div class="col-md-6">
                <div class="glass-card p-4 mb-3">
                    ...
                    <ul class="list-unstyled">
                        ...
                        <li><strong>Flask:</strong> {{ info.flask_version }}</li>
                        <li><strong>প্ল্যাটফর্ম:</strong> {{ info.platform }}</li>
                        <li><strong>প্রসেসর:</strong> {{ info.processor }}</li>
                        <li><strong>হোস্টনেম:</strong> {{ info.hostname }}</li>
                    </ul>
                </div>
            </div>
            <div class="col-md-6">
                <div class="glass-card p-4 mb-3">
                    <h5>রিসোর্স</h5>
                    <ul class="list-unstyled">
                        <li><strong>CPU কোর:</strong> {{ info.cpu_count }}</li>
                        <li><strong>CPU ফ্রিকোয়েন্সি:</strong> {{ info.cpu_freq_current }}</li>
                        <li><strong>মোট RAM:</strong> {{ info.total_ram }}</li>
                        <li><strong>উপলব্ধ RAM:</strong> {{ info.available_ram }}</li>
                        <li><strong>ফ্রি স্টোরেজ:</strong> {{ info.free_storage }}</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    return render_template_string(server_info_html, info=info)
